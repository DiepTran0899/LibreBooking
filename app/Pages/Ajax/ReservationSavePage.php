...
class ReservationSavePage extends SecurePage implements IReservationSavePage
{
    ...
    public function GetAttachments()
    {
        ...
        if ($this->AttachmentsEnabled()) {
            // Get regular file uploads
            $uploadedFiles = $this->server->GetFiles(FormKeys::RESERVATION_FILE);
            if (!empty($uploadedFiles)) {
                $attachments = array_merge($attachments, $uploadedFiles);
            }
            
            // Get captured images from camera
            ...
            if (!empty($capturedImages) && is_array($capturedImages)) {
                foreach ($capturedImages as $imageData) {
                    if (!empty($imageData['data']) && !empty($imageData['filename'])) {
                        ...
                        // Create a mock UploadedFile object for captured image
                        $uploadedFile = new CapturedImageFile($fileName, 'image/jpeg', strlen($imageContent), $imageContent, 'jpg');
    ...
    /**
     * Get attachment notes for existing files
     * @return array
     */
    public function GetAttachmentNotes()
    {
        $notes = $this->server->GetForm('attachment_notes');
