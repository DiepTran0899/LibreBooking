...
abstract class ReservationPage extends Page implements IReservationPage
{
    ...
    public function PageLoad()
    {
        ...
        // Get allowed attachment extensions and convert to accept attribute format
        $allowedExtensions = $config->GetKey(ConfigKeys::UPLOAD_RESERVATION_ATTACHMENT_EXTENSIONS);
        $acceptFormat = '';
        if (!empty($allowedExtensions)) {
            $extensions = array_map('trim', explode(',', $allowedExtensions));
            $acceptFormat = '.' . implode(',.', $extensions);
        }
        $this->Set('AllowedAttachmentExtensions', $acceptFormat);
        $this->Set('AllowedAttachmentExtensionsList', $allowedExtensions);
