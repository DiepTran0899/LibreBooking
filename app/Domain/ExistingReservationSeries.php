...
//        }
    ...
    /**
     * Update notes for existing attachments
     * @param array $attachmentNotes Array of fileId => note
     */
    public function UpdateAttachmentNotes($attachmentNotes)
    {
        if (!empty($attachmentNotes) && is_array($attachmentNotes)) {
            foreach ($attachmentNotes as $fileId => $note) {
                if (!empty($fileId) && array_key_exists($fileId, $this->attachmentIds)) {
                    $this->AddEvent(new AttachmentNoteUpdatedEvent($this, $fileId, $note));
                }
            }
        }
    }
