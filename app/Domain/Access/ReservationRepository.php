    ...
    public function LoadReservationAttachment($attachmentFileId)
    {
        ...
        if ($row = $reader->GetRow()) {
            ...
            $attachment = ReservationAttachment::Create(
                ...
                isset($row[ColumnNames::FILE_NOTE]) ? $row[ColumnNames::FILE_NOTE] : null
            );
...
class ReservationEventMapper
{
    ...
    private function __construct()
    {
        ...
        $this->buildMethods['AttachmentNoteUpdatedEvent'] = 'BuildAttachmentNoteUpdatedEvent';
    ...
    private function BuildAttachmentNoteUpdatedEvent(AttachmentNoteUpdatedEvent $event, ExistingReservationSeries $series)
    {
        return new AttachmentNoteUpdatedCommand($event);
    }
...
class AttachmentNoteUpdatedCommand extends EventCommand
{
    /**
     * @var AttachmentNoteUpdatedEvent
     */
    private $event;

    public function __construct(AttachmentNoteUpdatedEvent $event)
    {
        $this->event = $event;
    }
    ...
    public function Execute(Database $database)
    {
        $database->Execute(new UpdateReservationAttachmentNoteCommand($this->event->FileId(), $this->event->Note()));
    }
}
